...
function buildPreview(
  ...
  const who = role === "system" ? "System" : senderIsBuyer ? "You" : "Vendor";
...
export async function fetchBuyerListingConversations() {
  try {
    ...
    const vendorIds = Array.from(
      new Set(rows.map((r) => String(r.vendor_user_id ?? "").trim()).filter(Boolean))
    );

    let businessMap = new Map<string, BusinessProfileRow>();
    ...
    if (vendorIds.length > 0) {
      ...
      businessMap = new Map(
        ((businessProfiles ?? []) as BusinessProfileRow[]).map((b) => [String(b.user_id), b])
      );
    ...
    for (const m of messages) {
      ...
      const isBuyerMessage = String(m.sender_user_id ?? "") === String(user.id);
      ...
      if (!isBuyerMessage && !isSystemMessage && createdAtMs > lastReadMs) {
    ...
    return rows
      ...
      .map((r) => {
        ...
        const latestSenderIsBuyer = String(latest?.sender_user_id ?? "") === String(user.id);
        const vendorId = String(r.vendor_user_id ?? "").trim();
        const biz = vendorId ? businessMap.get(vendorId) ?? null : null;

        const vendorName =
          String(biz?.business_name ?? "").trim() ||
          String(profileMap.get(vendorId) ?? "").trim() ||
          "Vendor";
        ...
        return {
          ...
          vendor_business_name: biz?.business_name ?? null,
          vendor_city: biz?.city ?? null,
          vendor_locality: biz?.locality ?? null,
          ...
          last_message_preview: latest
            ? buildPreview(latest.body, latest.message_type, latestSenderIsBuyer, latest.sender_role)
            : null,
