...
const unique = Array.from(
  new Map(
    rows.map((r) => [
      `${r.pincode}:${r.office_name}:${r.district_name}:${r.state_name}`,
      r,
    ])
  ).values()
);
...
console.log(`Unique post office rows: ${unique.length}`);

await upsertChunks("geo_post_offices", unique, "pincode,office_name,district_name,state_name");

console.log("✅ India Post post office import complete.");
