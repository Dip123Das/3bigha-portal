...
assert(layout.includes("<AppPrivacyShield>") && layout.indexOf("<AppPrivacyShield>") < layout.indexOf("<Stack"), "root privacy boundary does not cover rendered authentication and business surfaces");
...
assert(shield.includes("{children}") && shield.includes("privateState &&") && shield.includes('position: "absolute"') && shield.includes("top: 0") && shield.includes("bottom: 0"), "state-preserving privacy overlay is absent");
assert(shield.includes("accessibilityElementsHidden={privateState}") && shield.includes('"no-hide-descendants"'), "covered content remains exposed to accessibility services");
...
for (const forbidden of ["user.id", "access_token", "notification", "error.message", "console.", "AsyncStorage", "SecureStore", "fetch(", "supabase"]) {
