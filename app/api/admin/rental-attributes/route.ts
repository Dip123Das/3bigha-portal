    ...
    return reply(
      {
        ok: false,
        error:
          "This attribute is connected to other rental data and cannot be changed in that way.",
      },
      409
    );
  }
...
async function usageCounts(
  ...
  const [valuesResult, mappingsResult] = await Promise.all([
    valuesQuery,
    mappingsQuery,
  ]);

  const error =
    valuesResult.error || mappingsResult.error;
...
export async function PATCH(request: Request) {
  ...
  if (body.input_type !== existing.input_type) {
    return reply(
      {
        ok: false,
        error:
          "Input type is permanent and cannot be changed after creation.",
      },
      409
    );
  }
