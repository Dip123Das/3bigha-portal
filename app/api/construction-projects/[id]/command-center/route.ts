...
export async function GET(_req: Request, context: RouteContext) {
  try {
    ...
    console.error("Construction command center GET error:", error);
    ...
    return NextResponse.json(
      {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Unable to generate construction command center.",
      },
      { status: 500 },
    );
  }
...
export async function POST(req: Request, context: RouteContext) {
  try {
    ...
    console.error("Construction command center POST error:", error);
