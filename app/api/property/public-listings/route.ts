...
export async function GET(req: Request) {
  try {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      if (workingCols.includes("is_public")) query = query.eq("is_public", true);
      if (workingCols.includes("published_at")) query = query.not("published_at", "is", null);
      ...
      const m1 = String(res.error.message || "").match(/could not find the '([^']+)' column/i);
      const m2 = String(res.error.message || "").match(/column "([^"]+)" .* does not exist/i);
      ...
      return NextResponse.json(
        {
          error: res.error.message || "Could not resolve property_listings columns",
        },
        { status: 500 }
      );
    }
