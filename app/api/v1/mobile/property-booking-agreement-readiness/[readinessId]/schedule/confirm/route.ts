...
import {
  mobileFailure,
  mobileSuccess,
  type MobilePropertyBookingAgreementScheduleConfirmation,
  ...
  try {
    ...
    const body =
      (await request.json().catch(() => ({}))) as
        Partial<MobilePropertyBookingAgreementScheduleConfirmation>;
    ...
    const workspace =
      await confirmMobilePropertyBookingAgreementSchedule({
        ownerUserId: auth.user.id,
        confirmation: { readinessId },
      });
