...
import { buildMobilePropertyWorkspace } from "@/lib/mobile/server/property-workspace";
...
export async function GET(request: Request) {
  try {
    ...
    const workspace = await buildMobilePropertyWorkspace(
      auth.supabase,
      auth.user,
    );
    ...
    console.error("MOBILE_PROPERTY_WORKSPACE_FAILED", error);
    return NextResponse.json(
      mobileFailure(
        "PROPERTY_WORKSPACE_FAILED",
        "Your property workspace could not be prepared.",
        true,
      ),
