...
import BuyerConversationChatBox from "./buyer-conversation-chat-box";
...
function buildBackHref(contextType?: string | null, contextId?: string | null) {
  ...
  if (ct === "rfq") return `/dashboard/buyer/quote-compare/${encodeURIComponent(id)}`;
...
export default async function BuyerConversationPage({
    ...
    return (
      <div style={{ padding: 12 }}>
        ...
        <div style={{ marginTop: 12, display: "flex", gap: 12, flexWrap: "wrap" }}>
          ...
          <Link href="/dashboard/buyer/inbox" style={{ fontWeight: 800 }}>
            Buyer Inbox
          </Link>
  ...
  const role = "buyer";
  const counterpartUserId = String(conv.vendor_user_id ?? "");
  ...
  const biz = (bizRes.data ?? null) as BusinessProfileRow | null;
  ...
  const cleanPhone = digitsOnly(counterpartPhone);
    ...
    <div style={{ padding: 12, maxWidth: 1000, margin: "0 auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          ...
          {counterpartPhone ? (
            <a href={`tel:${counterpartPhone}`} style={{ fontWeight: 800, textDecoration: "none" }}>
              Call
            </a>
          ) : null}
          {whatsappHref ? (
      ...
      >
        <div><strong>Counterpart:</strong> {counterpartName}</div>
