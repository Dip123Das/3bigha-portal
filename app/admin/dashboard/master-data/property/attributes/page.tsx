...
export default function PropertyAttributesMasterPage() {
  ...
  const inputTypeLocked =
    mode === "edit" && connectedCount > 0;

  const unitLocked =
    mode === "edit" &&
    (editingAttribute?.listing_answer_count ?? 0) > 0;
  ...
  async function loadAttributes() {
    ...
    try {
      const response = await fetch("/api/admin/property-attributes", {
        method: "GET",
      ...
      setError(
        loadError instanceof Error
          ? loadError.message
          : "Failed to load property attributes."
      );
    } finally {
      setLoading(false);
    }
  ...
  async function saveAttribute() {
    ...
    const confirmed = window.confirm(
      [
        mode === "add"
          ? "Create this property attribute?"
          : "Save these property attribute changes?",
        "",
        `Name: ${name}`,
  ...
  async function changeStatus(attribute: AttributeRow) {
    ...
    const confirmed = window.confirm(
      `${action === "activate" ? "Activate" : "Deactivate"} ` +
        `attribute “${attribute.name}”?\n\n` +
        (nextStatus
          ? "It may again become available to future listing workflows."
          : "Historical values, mappings and listing answers will be preserved.")
    );
  ...
  return (
    <Container>
      <SectionHeader
        ...
        subtitle="Control reusable property questions, answer formats and units used by subtype mappings and listing forms."
      />
                ...
                <FieldLabel
                  ...
                  hint={
                    inputTypeLocked
                      ...
                      : "Choose how a property lister will answer."
                  }
                />
                <select
      ...
      {loading ? (
        <EmptyState message="Loading property attributes…" />
      ) : attributes.length === 0 ? (
        <EmptyState message="No property attributes are available." />
      ) : (
        <div
          style={{
