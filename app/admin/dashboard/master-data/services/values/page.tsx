...
type Summary = {
  ...
  service_specific_values: number;
};
...
export default function ServicesValuesPage() {
  const supabase = useMemo(() => getSupabaseBrowser(), []);
  const [attributes, setAttributes] = useState<Attribute[]>([]);
  const [services, setServiceTaxons] = useState<ServiceTaxon[]>([]);
  ...
  async function api(method: "GET" | "POST" | "PATCH", body?: unknown) {
    ...
    const response = await fetch("/api/admin/service-values", {
      method,
  ...
  async function load() {
    ...
    try {
      ...
      setServiceTaxons(Array.isArray(result.services) ? result.services : []);
      setValues(Array.isArray(result.values) ? result.values : []);
      setSummary(result.summary || null);
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "Failed to load Services Values.");
    } finally {
      setLoading(false);
    }
  }
  ...
  const activeServices = useMemo(
    () => services.filter((row) => row.is_active),
    [services]
  );

  const visibleValues = useMemo(() => values.filter((row) => {
    if (form.attribute_id && row.attribute_id !== form.attribute_id) return false;
    if (form.scope === "global" && row.service_taxon_id) return false;
    if (form.scope === "service" && form.service_taxon_id && row.service_taxon_id !== form.service_taxon_id) return false;
    if (form.scope === "service" && !form.service_taxon_id && !row.service_taxon_id) return false;
    return showInactive || row.is_active;
  }), [values, form.attribute_id, form.scope, form.service_taxon_id, showInactive]);
  ...
  function selectAttribute(id: string) {
    if (editingId) return;
    setForm((current) => ({ ...current, attribute_id: id, service_taxon_id: "" }));
    setAiSuggestions([]);
    setAiMessage("");
  }

  function selectScope(scope: "global" | "service") {
    if (editingId) return;
    setForm((current) => ({ ...current, scope, service_taxon_id: "" }));
    setAiSuggestions([]);
    setAiMessage("");
  }
  ...
  function beginCreate() {
    ...
    setForm((current) => ({ ...emptyForm, attribute_id: current.attribute_id, scope: current.scope, service_taxon_id: current.service_taxon_id }));
  ...
  function beginEdit(row: ValueRow) {
    ...
    setForm({
      ...
      scope: row.service_taxon_id ? "service" : "global",
      service_taxon_id: row.service_taxon_id || "",
      value: row.value,
      slug: row.slug,
  ...
  async function save() {
    ...
    try {
      if (!form.attribute_id) throw new Error("Step 1: Choose the parent Services Attribute.");
      if (form.scope === "service" && !form.service_taxon_id) throw new Error("Step 2: Choose the Individual Service for this scoped Value.");
      ...
      const payload = editingId
        ...
        : {
            ...
            service_taxon_id: form.scope === "service" ? form.service_taxon_id : null,
            value: form.value,
      ...
      setMessage(editingId ? "Services Value updated successfully." : "Services Value created successfully.");
      setEditingId(null); setSlugTouched(false);
      setForm((current) => ({ ...emptyForm, attribute_id: current.attribute_id, scope: current.scope, service_taxon_id: current.service_taxon_id }));
      setAiSuggestions([]); setAiMessage("");
      await load();
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "Could not save the Services Value.");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function toggle(row: ValueRow) {
    ...
    try {
      ...
      setMessage(row.is_active ? "Services Value deactivated. Its history remains preserved." : "Services Value reactivated.");
      await load();
    } catch (caught) {
  ...
  async function requestAi(task: "name_suggestions" | "description") {
    if (!selectedAttribute) { setAiMessage("Choose a Services Attribute first."); return; }
    setAiBusy(task === "name_suggestions" ? "names" : "description");
    setAiMessage("");
    if (task === "name_suggestions") setAiSuggestions([]);
    try {
      const token = await accessToken();
      const service = services.find((row) => row.id === form.service_taxon_id);
      const response = await fetch("/api/admin/master-description", {
        method: "POST",
        headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
        body: JSON.stringify({
          task, kind: "service_value", name: form.value,
          context: {
            name: form.value,
  ...
  const selectedService = services.find((row) => row.id === form.service_taxon_id);

  return (
    <Container>
      <SectionHeader title="Services → Values" subtitle="Create controlled choices for reusable Services Attributes." />
      ...
      <section style={{ padding: 14, borderRadius: 14, background: "#eff6ff", marginBottom: 14 }}>
        ...
        <div style={{ color: "#334155", marginTop: 5, lineHeight: 1.55 }}>
          Choose a controlled-choice Attribute, choose whether its answers are Global or Individual-Service-specific, then add one answer at a time.
          Examples include Online or On-site for Service Mode, Immediate or Within 24 hours for Response Time, and Bengali, Hindi or English for Languages Supported. Prices, availability and provider claims belong on listings—not here.
        </div>
      </section>
      ...
      {summary ? (
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14 }}>
          ...
          <Badge>Global {summary.global_values}</Badge><Badge>Individual Service {summary.service_specific_values}</Badge>
        </div>
      ) : null}
      ...
      <section style={{ border: "1px solid #e2e8f0", borderRadius: 14, padding: 14, marginBottom: 14 }}>
        <h3 style={{ margin: "0 0 5px" }}>Step 1 — Choose the parent Services Attribute</h3>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: 10, marginTop: 10 }}>
          ...
          <button type="button" disabled={Boolean(editingId)} onClick={() => selectScope("service")} style={{ ...button(form.scope === "service"), textAlign: "left", padding: 14 }}>
            Individual Service-specific Value<br /><small>Available only for one individual Service</small>
          </button>
        </div>
        {form.scope === "service" ? (
          <div style={{ marginTop: 10 }}>
            <select value={form.service_taxon_id} disabled={Boolean(editingId) || !form.attribute_id} onChange={(event) => updateForm("service_taxon_id", event.target.value)} style={control(Boolean(editingId) || !form.attribute_id)}>
              <option value="">Select an active individual Service</option>
              {activeServices.map((row) => <option key={row.id} value={row.id}>{row.name}</option>)}
            </select>
            {!activeServices.length ? <p style={{ color: "#b45309", margin: "8px 0 0" }}>No active individual Services are currently available.</p> : null}
          </div>
        ) : null}

        <h3 style={{ margin: "18px 0 5px" }}>Step 3 — {editingId ? "Edit" : "Add"} controlled Value</h3>
        <p style={{ color: "#64748b", margin: "0 0 12px" }}>
          {selectedAttribute ? `Adding an answer for ${selectedAttribute.name}${form.scope === "service" && selectedService ? ` for ${selectedService.name}` : " globally"}.` : "Choose an Attribute above before entering a Value."}
        </p>
        ...
        }} placeholder="Example: Online, On-site, Immediate, Bengali or Hindi" style={control()} />
      ...
      <section style={{ border: "1px solid #e2e8f0", borderRadius: 14, padding: 14 }}>
        ...
        {loading ? <p>Loading Services Values…</p> : !visibleValues.length ? <EmptyState message="No controlled Values are available in this selection." /> : (
          <div style={{ display: "grid", gap: 10 }}>
            {visibleValues.map((row) => {
              const attribute = attributes.find((item) => item.id === row.attribute_id);
              const group = services.find((item) => item.id === row.service_taxon_id);
      ...
      <section style={{ padding: 14, borderRadius: 14, background: "#eff6ff", marginTop: 14 }}>
        ...
        <div style={{ color: "#475569", marginTop: 5 }}>AI suggestions never save automatically. Permanent keys, parent Attributes and individual-Service scopes lock after creation. Deactivation preserves history. Controlled Values are reusable answers—not prices, availability, provider identity, qualifications, licences or quality claims.</div>
      </section>
    </Container>
  );
}
