...
function getOwnerLabel(row: OpportunityRow) {
  return (
    row.owner_name ||
    row.promoter_name ||
    row.company_name ||
...
function getUpdatedAt(row: OpportunityRow) {
...
export default function AdminInvestmentOpportunitiesPage() {
  ...
  async function load() {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(json?.error || "Failed to load investment opportunities.");
      }

      setItems(normalizeList(json));
    } catch (e: any) {
      setError(e?.message || "Failed to load investment opportunities.");
    } finally {
      setLoading(false);
    }
  }
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((row) => {
      ...
      const owner = String(getOwnerLabel(row)).toLowerCase();
  ...
  const counts = useMemo(() => {
    return {
      all: items.length,
      draft: items.filter((x) => getStatus(x) === "draft").length,
      pending_review: items.filter((x) => getStatus(x) === "pending_review").length,
      approved: items.filter((x) => getStatus(x) === "approved").length,
      rejected: items.filter((x) => getStatus(x) === "rejected").length,
    };
  }, [items]);
      ...
      <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Review, approve, reject, or send back submitted opportunities.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          <div className="md:col-span-2">
            ...
            <input
              ...
              placeholder="Search by title, owner, slug, sector, location, or id"
                ...
                filteredItems.map((row) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-4 align-top text-sm text-gray-700">
                        {fmtDate(getUpdatedAt(row))}
                      </td>

                      <td className="px-4 py-4 align-top text-right">
                        <Link
                          href={`/admin/dashboard/investment/opportunities/${encodeURIComponent(
                            String(row.id)
                          )}`}
