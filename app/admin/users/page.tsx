...
export default async function AdminUsersPage({searchParams}:{searchParams?:Params}){
  ...
  const highest=all.filter(({business,complimentary})=>["enterprise","lifetime"].includes(complimentary?.active?complimentary.plan:business.subscription_plan||"")).length;
  ...
  return <main className={styles.page}>
    ...
    <form method="get" className={styles.filters}>
      ...
      <select className={styles.field} name="plan" defaultValue={plan}><option value="">All plans</option><option value="none">No plan</option><option value="starter">Starter</option><option value="professional">Professional</option><option value="enterprise">Enterprise</option><option value="lifetime">Lifetime</option></select>
      ...
      {selected?(()=>{
        ...
        return <section className={styles.detail} data-workspace={activeWorkspace}>
          ...
          <nav className={styles.quickActions} aria-label="Founder quick actions">
            <a href="#founder-controls">Manage account</a>
            <a href="#subscription-control">Grant plan</a>
          ...
          <section className={styles.operatingCentre}>
            ...
            <nav className={styles.workspaceTabs} aria-label="Member operating workspaces">
              ...
              ].map(([key,label,description])=><a key={key} href={workspaceHref(key,profile.id)} className={activeWorkspace===key?styles.workspaceTabActive:styles.workspaceTab}><strong>{label}</strong><span>{description}</span></a>)}
            </nav>
            <div className={styles.externalTools}>
              ...
              <a href={workspaceHref(activeWorkspace,profile.id)}>Refresh selected workspace</a>
            </div>
          </section>
          <div className={styles.detailGrid}>
            <article id="identity-panel" className={`${styles.panel} ${styles.identityWorkspace}`}><h3>Identity</h3><div className={styles.kv}><span>Primary role</span><strong>{clean(profile.role)}</strong></div><div className={styles.kv}><span>Requested role</span><strong>{clean(profile.requested_role)}</strong></div><div className={styles.kv}><span>Approval</span><strong>{clean(profile.approval_status)}</strong></div><div className={styles.kv}><span>Member group</span><strong>{group}</strong></div></article>
            <article id="business-panel" className={`${styles.panel} ${styles.businessWorkspace}`}><h3>Account & business</h3><div className={styles.kv}><span>Status</span><strong>{clean(accountStatus)}</strong></div><div className={styles.kv}><span>Status note</span><strong>{profile.account_status_reason||"No note"}</strong></div><div className={styles.kv}><span>Joined</span><strong>{profile.created_at?new Date(profile.created_at).toLocaleDateString("en-IN"):"—"}</strong></div></article>
            <article className={`${styles.panel} ${styles.subscriptionWorkspace}`}><h3>Subscription</h3><div className={styles.kv}><span>Plan</span><strong>{clean(effectivePlan)}</strong></div><div className={styles.kv}><span>Status</span><strong>{clean(effectiveStatus)}</strong></div><div className={styles.kv}><span>Expiry</span><strong>{complimentary?.active?(complimentary.expires_at?new Date(complimentary.expires_at).toLocaleDateString("en-IN"):"Never"):business.subscription_expires_at?new Date(business.subscription_expires_at).toLocaleDateString("en-IN"):"Not recorded"}</strong></div><div className={styles.kv}><span>Granted by</span><strong>{complimentary?.active?complimentary.granted_by||"Admin ID not recorded":"Payment system / not recorded"}</strong></div><div className={styles.kv}><span>Grant reason</span><strong>{complimentary?.active?complimentary.reason||"Not recorded":"Not complimentary"}</strong></div></article>
            <article id="location-panel" className={`${styles.panel} ${styles.geographyWorkspace}`}><h3>LGD Geography</h3><div className={styles.kv}><span>Country</span><strong>India</strong></div><div className={styles.kv}><span>State</span><strong>{stateNames.get(stateId)||"Not recorded"}</strong></div><div className={styles.kv}><span>District</span><strong>{districtNames.get(districtId)||"Not recorded"}</strong></div></article>
            <article className={`${styles.panel} ${styles.verificationWorkspace}`}><h3>Login & verification</h3><div className={styles.kv}><span>Email verified</span><strong>{emailVerified?"Yes":"No"}</strong></div><div className={styles.kv}><span>Phone verified</span><strong>{phoneVerified?"Yes":"No"}</strong></div><div className={styles.kv}><span>Last login</span><strong>{lastLogin?new Date(lastLogin).toLocaleString("en-IN"):"No login recorded"}</strong></div><div className={styles.kv}><span>Auth account created</span><strong>{authCreated?new Date(authCreated).toLocaleString("en-IN"):"Not recorded"}</strong></div></article>
            <article className={`${styles.panel} ${styles.panelWide} ${styles.verificationWorkspace}`}><h3>Readiness breakdown</h3><div className={styles.readinessGrid}>{readinessItems.map(([label,complete,weight])=><div key={String(label)} className={complete?styles.readinessComplete:styles.readinessMissing}><span>{complete?"✓":"!"}</span><strong>{label}</strong><small>{complete?`Recorded · ${weight}%`:`Missing · ${weight}%`}</small></div>)}</div></article>
            <article className={`${styles.panel} ${styles.panelWide} ${styles.timelineWorkspace}`}><h3>Member timeline</h3><div className={styles.timeline}><div><i/><span>Account created {profile.created_at?new Date(profile.created_at).toLocaleString("en-IN"):"date unavailable"}</span></div><div><i/><span>Identity status: {clean(profile.approval_status)}</span></div>{complimentary?.active?<div><i/><span>Complimentary {clean(complimentary.plan)} granted {complimentary.granted_at?new Date(complimentary.granted_at).toLocaleString("en-IN"):""} · Reason: {complimentary.reason||"Not recorded"}</span></div>:null}</div></article>
            <article id="founder-controls" className={`${styles.panel} ${styles.panelFull} ${styles.controlsWorkspace}`}><h3>Founder controls</h3><div className={styles.actions}>
              ...
              {profile.id!==user.id&&profile.role!=="master_admin"?<div id="subscription-control" className={styles.grantBox}><strong>Grant complimentary subscription</strong><form action="/api/admin/member-subscription" method="post" className={styles.inlineForm}><input type="hidden" name="user_id" value={profile.id}/><select name="plan" defaultValue="enterprise" required><option value="starter">Starter</option><option value="professional">Professional</option><option value="enterprise">Enterprise</option><option value="lifetime">Lifetime</option></select><input name="reason" placeholder="Internal reason" required/><input name="expires_on" type="date"/><button className={styles.approve}>Grant without payment</button></form></div>:null}
            </div></article>
          </div>
        </section>
