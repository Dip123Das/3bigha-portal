...
export default async function AdminUsersPage({searchParams}:{searchParams?:Params}){
  ...
  const all=(profilesRes.data||[]).map((profile:any)=>{
    ...
    return{profile,business,complimentary,group:classify(profile.role||"",profile.requested_role||""),stateId:business.geo_state_id||profile.geo_state_id||"",districtId:business.geo_district_id||profile.geo_district_id||""};
  });
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.workspace}>
      <aside className={styles.memberList}><div className={styles.listHeader}>{filtered.length} matching members</div>
        {filtered.map(({profile,business,group,complimentary})=>{const params=new URLSearchParams(preserve);params.set("member",profile.id);const selectedNow=selected?.profile.id===profile.id;const planName=complimentary?.active?complimentary.plan:business.subscription_plan||"free";return <a key={profile.id} href={`/admin/users?${params.toString()}`} className={`${styles.memberLink} ${selectedNow?styles.memberLinkActive:""}`}><span className={styles.avatar}>{(profile.full_name||business.business_name||"M").charAt(0).toUpperCase()}</span><span className={styles.memberCopy}><strong>{profile.full_name||business.business_name||"Unnamed member"}</strong><small>{profile.email||"No email"}</small></span><span className={styles.listMeta}><span>{group}</span><small>{clean(planName)}</small></span></a>})}
      </aside>

      {selected?(()=>{
        const{profile,business,complimentary,group,stateId,districtId}=selected;
        ...
        return <section className={styles.detail}>
          ...
          <div className={styles.detailGrid}>
            ...
            <article className={`${styles.panel} ${styles.panelFull}`}><h3>Founder controls</h3><div className={styles.actions}>
