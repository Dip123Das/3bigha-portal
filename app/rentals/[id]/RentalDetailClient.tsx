...
function normalizeRow(x: any): RentalPublicRow | null {
...
export default function RentalDetailClient({
  ...
  id: string;
  initialRow: RentalPublicRow;
}) {
  const loading = false;
  const err: string | null = null;
  const row: RentalPublicRow = initialRow;
...
const aiRecommendations = buildRecommendations({
  ...
  minPrice: row?.rate ? Number(row.rate) * 0.7 : null,
  maxPrice: row?.rate ? Number(row.rate) * 1.3 : null,
  userIntent: "rental discovery",
});
