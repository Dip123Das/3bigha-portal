...
export function NotificationDeviceCard({ session }: { session: Session }) {
  ...
  const actionLabel = busy ? "Updating work alerts" : state?.enabled ? "Turn off work alerts on this device" : "Enable important work alerts";
  return <View accessible={false} style={styles.card}><Text style={styles.kicker}>THIS DEVICE</Text><Text accessibilityRole="header" style={styles.title}>Work alerts</Text>{pending && <Text accessibilityLiveRegion="polite" style={styles.pending}>Waiting for internet · saved on this device</Text>}<Text accessibilityLiveRegion="polite" style={styles.body}>{message}</Text><Pressable accessibilityLabel={actionLabel} accessibilityRole="button" accessibilityState={{ disabled: busy, busy }} disabled={busy} hitSlop={8} onPress={() => void (state?.enabled ? disable() : enable())} style={[styles.action, busy && styles.disabled]}><Text style={styles.actionText}>{busy ? "Updating…" : state?.enabled ? "Turn off on this device" : "Enable important alerts"}</Text></Pressable></View>;
}
