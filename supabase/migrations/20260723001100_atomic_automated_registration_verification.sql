   ...
   */
     ...
     or (
       ...
       and (
         ...
         or not v_registration_complete
       )
     )
     or (
       v_business_required
...
comment on function
  public.evaluate_automated_registration_verification()
is
  'Atomically evaluates the authenticated member from stored registration evidence, writes the canonical verification state and audit event, and makes an eligible dashboard ready. It never assigns roles, changes subscriptions, projects legacy approval, or activates dashboard access.';
