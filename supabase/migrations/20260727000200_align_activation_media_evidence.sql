...
begin
  ...
  /*
   * Canonical onboarding media is stored in business_media_json.
   *
   * Keep the legacy dedicated columns as compatible fallbacks,
   * but do not reject valid evidence merely because those older
   * projection columns were not populated.
   */
  v_selfie_present :=
    (
    ...
    or exists (
      ...
      from jsonb_array_elements(
        case
          when jsonb_typeof(v_business_media_json) = 'array'
            then v_business_media_json
          else '[]'::jsonb
        end
      ) asset
      where lower(
  ...
  /*
   * A stored canonical capture is the authoritative completion
   * signal when the older status projection is still missing.
   */
  if v_selfie_present
     and v_selfie_status in ('', 'missing', 'pending') then
    v_selfie_status := 'verified';
  end if;

  if v_workplace_present
     and v_workplace_status in ('', 'missing', 'pending') then
  ...
  v_evidence_snapshot := jsonb_build_object(
    ...
    'workplace_evidence_present',
      v_workplace_present,
    'canonical_business_media_count',
      case
        when jsonb_typeof(v_business_media_json) = 'array'
          then jsonb_array_length(v_business_media_json)
        else 0
      end,
    'document_required',
      v_document_required,
